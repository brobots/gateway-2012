...
typedef enum
{
  ...
  ERR_BadEncryption        = -8,  //0xF8 Dick's add -- Bad encryption of file header.
...
typedef enumWord // NOTE the "enumWord" -- ROBOTC compiler will assign it 16-bit value!!!
{
  ...
  #define ERR_COMM_CHAN_INVALID   ((short) ioRsltCommChannelBad)
...
typedef enum                          // Index in status icon collection file
{
...
typedef enum                          // String index in text string file
{
  TXT_GENERAL_EMPTY,
                                            // BlueTooth connect
  ...
  TXT_FILERUN_FILE_EncryptionError,         // "Bad Encryption!"
