...
void setServo(tMotor servoPort = port6, byte position = 0)
{
...
void untilRotations(long rotations, tSensors sensorPort)// = 1, tSensors sensorPort = in2)
{
  SensorValue[sensorPort] = 0;

  while(SensorValue[sensorPort] < (abs(rotations) * 360)  &&  SensorValue[sensorPort] > (abs(rotations) * (360 * -1))){wait1Msec(1);}
}
...
void untilEncoderCounts(long distance, tSensors sensorPort)// = 360, tSensors sensorPort = in2)
{
  SensorValue[sensorPort] = 0;

  while(SensorValue[sensorPort] < (abs(distance))  &&  SensorValue[sensorPort] > (abs(distance) * -1)){wait1Msec(1);}
}
...
void moveStraightForTime(int seconds, tSensors rightEncoderPort, tSensors leftEncoderPort)// = 5, tSensors rightEncoderPort = in2, tSensors leftEncoderPort = in3)
{
  ...
  long timeStart = ((int)nPgmTime / 1000);
